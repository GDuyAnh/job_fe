    ...
    >
      <div class="bg-transparent text-center">
        <h2 class="text-xl font-semibold text-white mb-2">
          Preview - Công ty chưa được duyệt
        </h2>
      </div>
    </div>
      ...
      <UContainer>
        <div class="py-4">
          <div class="flex items-center gap-4">
            ...
            <UBadge color="orange" variant="soft" class="ml-2">
              {{ $t('dashboard.admin.companies.status.pending') }}
            </UBadge>
          </div>
        </div>
      </UContainer>
...
const fetchCompanyDetail = async () => {
  ...
  try {
    ...
    // Use the admin endpoint to get company detail including unapproved companies
    const response = await $api.company.adminGetCompanyDetail(companyId)
...
const contactAdmin = () => {
  // Could open a support chat or email
  alert('Vui lòng liên hệ quản trị viên để duyệt công ty này.')
}
